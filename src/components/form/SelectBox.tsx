...
const SelectBox = ({ index, type, pageMode }: ISelectBox) => {
  ...
  return (
    <Container $pageMode={pageMode}>
      ...
      {pageMode === "survey" ? (
        <select value={type} onChange={handleTypeChange} tabIndex={0}>
          <option value="textShort" aria-label="단답형">
            단답형
          </option>
          <option value="textLong" aria-label="장문형">
            장문형
          </option>
          <option value="radio" aria-label="객관식 질문">
            객관식 질문
          </option>
          <option value="checkbox" aria-label="체크박스">
            체크박스
          </option>
          <option value="dropdown" aria-label="드롭다운">
            드롭다운
          </option>
        </select>
      ) : null}
        ...
        type === "dropdown" && (
          <>
            ...
            <select
              ...
              tabIndex={pageMode === "submit" ? -1 : 0}
            >
              <option value="" aria-label="선택" disabled>
                선택
              </option>
              {optionList.map(
                (option) =>
